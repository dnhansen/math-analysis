...
\definecolor{arrowcolor}{HTML}{a84e32}
...
\end{equation*}
%
for all $x,y \in S$. The space of Lipschitz functions $(S,\rho) \to \reals$ is denoted $\lip(S,\rho)$, and the subspace of bounded functions $\lipbounded(S,\rho)$. We also denote the space of continuous functions that are uniformly bounded with respect to $\rho$ by $C_u(S,\rho)$.
...
If $(X,\calE,\mu)$ is a measure space, then $\mu$ induces a linear functional $\phi_\mu$ on $\calL^1(\mu)$ by
%
\begin{equation*}
    \phi_\mu(f) = \int_X f \dif \mu.
\end{equation*}
%
If $\mu$ is finite and $X$ is a topological space, then $\mu$ in particular induces a functional $\phi_\mu$ on $C_b(X)$. We also use the notation $\mu(f)$ or just $\mu f$ for $\phi_\mu(f)$.
...
\begin{theorem}
    ...
    \begin{enumthm}
        ...
        \item \label{enum:portmanteau-uniformly-continuous-universal} For all metrics $\rho$ on $S$, $P_n f \to Pf$ as $n\to\infty$ for all $f \in C_u(S,\rho)$.

        \item \label{enum:portmanteau-uniformly-continuous-existence} There exists a metric $\rho$ on $S$ such that $P_n f \to Pf$ as $n\to\infty$ for all $f \in C_u(S,\rho)$.

        \item \label{enum:portmanteau-lipschitz-universal} For all metrics $\rho$ on $S$, $P_n f \to Pf$ as $n\to\infty$ for all $f \in \lipbounded(S,\rho)$.

        \item \label{enum:portmanteau-lipschitz-existence} There is a metric $\rho$ on $S$ such that $P_n f \to Pf$ as $n\to\infty$ for all $f \in \lipbounded(S,\rho)$.

        \item \label{enum:portmanteau-closed} $\limsup_{n\to\infty} P_n(F) \leq P(F)$ for all closed $F \subseteq S$.

        \item \label{enum:portmanteau-open} $\liminf_{n\to\infty} P_n(G) \geq P(G)$ for all open $G \subseteq S$.
...
\newcommand{\enumnumber}[1]{\textit{(#1)}}

\begin{proof}
We prove the following implications:
%
\begin{equation*} % TODO: Those are CM arrows, not KP arrows!
    \tikz[%remember picture, 
    overlay]{
    \draw[black,dashed,rounded corners] (2,-1) rectangle (2.95,-2.9);
    }
    \begin{tikzcd}
        & \enumnumber{i}
            \ar[d, Rightarrow, arrowcolor] \\
        & |[alias=two]| \enumnumber{ii}
            \ar[dl, Rightarrow, arrowcolor] \\
        \enumnumber{iii}
            \ar[dr, Rightarrow, arrowcolor]
        && |[alias=four]| \enumnumber{iv} \\
        & |[alias=five]| \enumnumber{v}
            \ar[d, Rightarrow] \\
        & \enumnumber{vi}
            \ar[d, Leftrightarrow] \\
        & \enumnumber{vii}
            \ar[d, Rightarrow] \\
        & \enumnumber{viii}
            \ar[uuuuuu, Rightarrow, bend right]
        \ar[dr, Rightarrow, from=two, to=four, arrowcolor]
        \ar[dl, Rightarrow, from=four, to=five, arrowcolor]
    \end{tikzcd}
\end{equation*}
%
The implications marked in red are trivial. Notice that the two existential claims follow from the the two universal claims since $S$ is assumed metrisable.

\begin{proofsec}
    \item[\subcref{enum:portmanteau-lipschitz-existence} $\implies$ \subcref{enum:portmanteau-closed}]
    Let $\rho$ be a metric on $S$ in accordance with the hypothesis, and let $g_k(x) = (1 - k\rho(x,F)) \join 0$ and $F_k = \set{x \in S}{\rho(x,F) < 1/k}$ for $k \in \naturals$. First notice that $g_k$ is Lipschitz, since
    %
    \begin{align*}
        \abs{ g_k(x) - g_k(y) }

...
\end{proposition}
%
In particular, by exchanging $f$ and $\alpha$ we find that
%
\begin{equation*}
...
\begin{proof}
    Since $\phi$ is bijective it is strictly monotonic, so it induces an increasing bijection $\calP'[c,d] \to \calP'[a,b]$ given by $(P, T) \mapsto (\phi(P), \phi(T))$.

    Now let $(P,T) \in \calP'[c,d]$ with $P = \{y_0, \ldots, y_n\}$ and $T = \{s_1, \ldots, s_n\}$ and write $x_i = \phi(y_i)$ and $t_i = \phi(s_i)$. Assuming for definiteness that $\phi$ is increasing, we have
    %
    \begin{equation*}
    ...
    \end{equation*}
    %
    Since the map $(P,T) \mapsto (\phi(P),\phi(T))$ is an increasing bijection, each side above converges to the corresponding integral, proving the claim.
\end{proof}
...
\begin{proof}
    ...
    \end{equation*}
    %
    and since $\epsilon$ was arbitrary, it follows from Riemann's condition that $f \in \integrable[\alpha]{a,b}$.
\end{proof}

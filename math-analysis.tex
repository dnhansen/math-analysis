...
\begin{theorem}[The multidimensional mean value theorem]
...
\begin{theorem}[Criterion for differentiability]
    ...
    Let $A \subseteq \reals^d$, let $a$ be an inner point of $A$, and $f \colon A \to \reals^m$. If one of the partial derivatives $D_1 f, \ldots, D_d f$ exists at $a$, and the remaining $d-1$ exist in a neighbourhood of $a$ and are continuous at $a$, then $f$ is differentiable at $a$.
\end{theorem}

\newcommand{\grad}{\nabla}

\begin{proof}
    We may assume that $m = 1$ by \cref{lem:differentiability-coordinate-functions}, and we may also assume that the partial derivaties $D_2 f, \ldots, D_d f$ exist in a neighbourhood $U$ of $a$. Let $h = (h_1, \ldots, h_d) \in \reals^d$, and for $k = 1, \ldots, d$ let $h^{(k)} = \sum_{j=1}^k h_j e_j$. Notice that $h^{(d)} = h$. Choose $h$ small enough such that $a + h^{(k)} \in U$ for all $k$.
    
    In the case $k = 1$, the existence of $D_1 f(a)$ yields the existence of a function $\delta_a \colon U - a \to \reals$, continuous at $0$, such that
    %
    \begin{equation*}
        f(a + h_1 e_1) - f(a)
            = D_1 f(a) h_1 + \delta_a(h_1),
        \quad \text{and} \quad
        \lim_{h_1 \to 0} \frac{\delta_a(h_1)}{h_1} = 0.
    \end{equation*}
    %
    Defining a function $R_1$ on a neighbourhood of $0$ by $R_1(h) = \delta_a(h_1)/h_1$, we thus have $R_1(h) \to 0$ as $h \to 0$, and
    %
    \begin{equation*}
        f(a + h^{(1)}) - f(a)
            = D_1 f(a) h_1 + R_1(h) h_1.
    \end{equation*}
    
    Next let $k \geq 2$, and let $I_k \subseteq \reals$ be an open interval containing $0$ and $h_k$ such that the map
    %
    \begin{align*}
    ...
    \end{equation*}
    %
    By continuity of $D_k$ at $a$, there is a function $R_k$ defined on a neighbourhood of $0$ such that
    %
    \begin{equation*}
        D_k f(a + h^{(k-1)} + s_k e_k)
            = D_k f(a) + R_k(h).
    \end{equation*}
    %
    ...
    \begin{align*}
        f(a+h) - f(a)
            ...
            &= f(a + h^{(1)}) - f(a) + \sum_{k=2}^d \bigl( g_k(h_k) - g_k(0) \bigr) \\
            &= \sum_{k=1}^d D_k f(a) h_k + \sum_{k=1}^d R_k(h) h_k \\
            &= \inner{\grad f(a)}{h} + \epsilon_a(h),
    \end{align*}
    %
    where $\epsilon_a(h) = \sum_{k=1}^d R_k(h) h_k$ is $o(h)$ as $h \to 0$. Hence $f$ is differentiable at $a$.
\end{proof}
...
Let $I \subseteq \reals$ be an interval, let $a \in I$, and let $f \colon I \to \complex$ be $k$ times differentiable at $a$. The \emph{Taylor polynomial} of order $k$ of $f$ at $a$ is the function $P_k \colon \reals \to \complex$ given by
%
\begin{equation*}
    P_k(x)
...
\end{equation*}
%
The $k$th \emph{remainder} is the function $R_k \colon I \to \complex$ given by $R_k(x) = f(x) - P_k(x)$. Notice that $f$ and $P_k$ agree at $a$, and so do their first $k$ derivatives. Furthermore, we have
%
\begin{equation*}
    R_{k-1}(x) - R_k(x)
        = P_k(x) - P_{k-1}(x)
...
% https://math.stackexchange.com/questions/2585752/application-of-the-mean-value-theorem-to-a-taylor-series/2586741#2586741
\begin{proposition}[Peano's form of the remainder]
    If $f \colon I \to \complex$ is $k \geq 1$ times differentiable at $a \in I$, then
    %
    \begin{equation*}
...
\end{proposition}
%
Note that the hypothesis implies that $f$ is $k-1$ times differentiable in a neighbourhood of $a$ in $I$.

\begin{proof}
    It suffices to prove the claim when $a = 0$. We first prove that if $g \colon I \to \complex$ is such that $g(0) = g'(0) = \cdots = g^{(k)}(0) = 0$, then $g(x)/x^k \to 0$ as $x \to 0$. For $k = 1$ this is just the definition of the derivative, namely
    %
    \begin{equation*}
        0
            = g'(0)
    ...
    \end{equation*}
    %
    since $g(0) = 0$. Assume that $k > 1$ and notice that, by induction, $g'(x)/x^{k-1} \to 0$ as $x \to 0$. If $x \neq 0$, then the mean value theorem furnishes a $\xi$ between $0$ and $x$ such that $g(x) = g'(\xi) x$. It follows that
    %
    \begin{equation*}
        \frac{g(x)}{x^k}
            = \frac{g'(\xi)}{x^{k-1}}
    ...
    \end{equation*}
    %
    as $x \to 0$, since then also $\xi \to 0$.

    To prove the proposition, simply let $g = R_k$.
\end{proof}


\begin{proposition}[Integral form of the remainder]
    If $I$ is the closed interval between $a$ and $x$, and if $f \colon I \to \complex$ is $k$ times differentiable on $I$ and $f^{(k)}$ is absolutely continuous, then
    %
    \begin{equation*}
        R_k(x)
            = \frac{1}{k!} \int_a^x (x-t)^k f^{(k+1)}(t) \dif t.
    \end{equation*}
\end{proposition}
%
Since $f^{(k)}$ is absolutely continuous, $f^{(k+1)}$ exists almost everywhere on $I$.

\begin{proof}
    Denote the integral above by $I_k$. First note that
    %
    \begin{equation*}
        R_0(x)
            = f(x) - P_0(x)
    ...
    \end{equation*}
    %
    easily follows by integration by parts.\footnote{See Folland Exercise~3.35.}
\end{proof}


\begin{proposition}[Mean value form of the remainder]
    Let $I$ be the closed interval between $x$ and $a$, and let $f \in C^k(I,\complex)$ such that $f^{(k+1)}$ exists on $\interior{I}$. If $\phi \colon I \to \complex$ is continuous and $\phi'$ exists and is nonzero on $\interior{I}$, then there is a $\xi \in \interior{I}$ such that
    %
    \begin{equation*}
        R_k(x)
            = \frac{f^{(k+1)}(\xi)}{k!} (x-\xi)^k \frac{\phi(x) - \phi(a)}{\phi'(\xi)}.
    \end{equation*}
\end{proposition}

\begin{proof}
    Define a function $g \colon I \to \complex$ by\footnote{Notice that $g(t)$ is just the $k$th remainder of the Taylor expansion of $f$ around $t$ evaluated at $x$.}
    %
    \begin{equation*}
        g(t)
            = f(x) - \sum_{j=0}^k \frac{f^{(j)}(t)}{j!} (x-t)^j.
    \end{equation*}
    %
    Then $g$ is continuous on $I$ and differentiable on $\interior{I}$, and $g(x) = 0$ and $g(a) = R_k(x)$. Notice that
    %
    \begin{equation*}
        g'(t)
            = f'(t) - \sum_{j=1}^k \frac{1}{j!} \bigl( f^{(j+1)}(t) (x-t)^j - j f^{(j)}(t) (x-t)^{j-1} \bigr)
            = - \frac{f^{(j+1)}(t)}{j!} (x-t)^j,
    \end{equation*}
    %
    where the second equality follows since the sum is telescoping. The mean value theorem now furnishes a $\xi \in \interior{I}$ such that
    %
    \begin{equation*}
        - R_k(x)
            = - g(a)
            = g(x) - g(a)
            = \frac{\phi(x) - \phi(a)}{\phi'(\xi)} g'(\xi),
    \end{equation*}
    %
    as desired.
\end{proof}
...
TODO can this be generalised to uniform spaces?

\begin{definition}[Convergence in measure]
    Let $(X,\calE,\mu)$ be a measure space and $(S,\rho)$ a separable metric space. Let $f,f_1,f_2,\ldots$ be $\calE$-$\borel(S)$-measurable functions. We say that $(f_n)$ \emph{converges to $f$ in $\mu$-measure} if for every $\epsilon > 0$,
    %
    \begin{equation*}
        \lim_{n\to\infty} \mu\bigl( \{ \rho(f_n,f) > \epsilon \}\bigr) = 0.
    \end{equation*}
    %
    Furthermore, $(f_n)$ is called a \emph{Cauchy sequence in $\mu$-measure} if, for every $\epsilon > 0$,
    %
    \begin{equation*}
        \lim_{m,n\to\infty} \mu\bigl( \{ \rho(f_m,f_n) > \epsilon \} \bigr) = 0.
    \end{equation*}
\end{definition}
%
Note that separability (i.e. second countability) of $S$ is necessary to ensure that the maps $\rho(f_n,f)$ and $\rho(f_m,f_n)$ are measurable.

TODO generalise prop below
...
\begin{lemma}
    ...
    Let $(X,\calE,\mu)$ be a measure space and $(S,\rho)$ a complete separable metric space, and let $(f_n)_{n\in\naturals}$ be a sequence in $\measurable(\calE,\borel(S))$. If there exists a sequence $(\epsilon_n)_{n\in\naturals}$ of strictly positive numbers such that
    %
    \begin{equation}
        \label{eq:convergence-in-measure-complete-lemma}
        \sum_{n=1}^\infty \epsilon_n < \infty
        \quad \text{and} \quad
        \sum_{n=1}^\infty \mu \bigl( \{ \rho(f_{n+1},f_n) > \epsilon_n \} \bigr) < \infty,
    \end{equation}
    %
    then there exists a function $f \in \measurable(\calE,\borel(S))$ such that $(f_n)$ converges to $f$ both $\mu$-a.e. and in $\mu$-measure.
\end{lemma}

\begin{proof}
    For $n \in \naturals$, denote the set $\{ \rho(f_{n+1},f_n) > \epsilon_n \}$ by $E_n$, and define sets $F_k = \bigunion_{n=k}^\infty E_n$ and $F = \bigintersect_{k\in\naturals} F_k$. Notice that $F = \limsup_{n\to\infty} E_n$, so the first Borel--Cantelli lemma implies that $\mu(F) = 0$.
    ...
    \begin{equation}
        ...
        \rho(f_m,f_n)
            \leq \sum_{i=n}^{m-1} \rho(f_{i+1},f_i)
            \leq \sum_{i=n}^\infty \rho(f_{i+1},f_i).
    \end{equation}
    %
    ...
    \end{equation*}
    %
    The right-hand side converges to zero as $n \to \infty$, which shows that $(f_n(x))$ is a Cauchy sequence in $S$ for $x \in F_k^c$, hence for $x \in F^c$. Letting $f = \lim_{n\to\infty} f_n \indicator{F^c}$ we thus find that $(f_n)$ converges to $f \in \measurable(\calE)$ $\mu$-a.e.
    ...
    \end{equation*}
    %
    $\mu$-a.e. Now let $\epsilon > 0$, and choose an $N \in \naturals$ such that $\sum_{i=N}^\infty \epsilon_i \leq \epsilon$. For $n \geq N$, $\rho(f_n,f) > \epsilon$ then implies that
    %
    \begin{equation*}
    ...
    \end{equation*}
    %
    $\mu$-a.e., which in turn implies that $\epsilon_i < \rho(f_{i+1},f_i)$ $\mu$-a.e. for some $i \geq n$. Hence it follows that
    %
    \begin{equation*}
        \mu\bigl( \{ \rho(f_n,f) > \epsilon \}\bigr)
...
\begin{theorem}[Completeness of convergence in measure]
    Let $(X,\calE,\mu)$ be a measure space and $(S,\rho)$ a complete separable metric space, and let $(f_n)_{n\in\naturals}$ be a sequence in $\measurable(\calE,\borel(S))$ that is Cauchy in $\mu$-measure. Then there exists a function $f \in \measurable(\calE,\borel(S))$ such that $f_n \to f$ in $\mu$-measure. Furthermore, $(f_n)$ has a subsequence that converges to $f$ $\mu$-a.e.
\end{theorem}

\begin{proof}
    Choose a subsequence $(g_k) = (f_{n_k})$ of $(f_n)$ such that
    %
    \begin{equation*}
        \mu \bigl( \{ \rho(g_{k+1}, g_k) > 2^{-k} \} \bigr)
            \leq 2^{-k}.
    \end{equation*}
    %
    \Cref{thm:convergence-in-measure-lemma} then implies the existence of a function $f \in \measurable(\calE,\borel(S))$ such that $g_k \to f$ in $\mu$-measure. That is, $(f_n)$ is Cauchy and has a subsequence that converges to $f$, to it itself converges to $f$.
\end{proof}
...
\begin{enumerate}
    ...
    \end{equation*}
    %
    defines an outer measure $\mu^*$ on $X$. We say that $\mu^*$ is the outer measure \emph{generated by $\mu$}. We note that $\mu^*(A) \leq \mu(A)$ for all $A \in \calJ$.
\end{enumerate}
\end{remarkbreak}
...
\end{theorem}
%
We have stated these results with (as far as we know) minimal assumptions, but we shall only use (iii) and (v) in the sequel, and only for outer measures.

\begin{proof}
    (i) Clearly $\calM(\mu)$ contains $X$ and is closed under complements. For $A,B \in \calM(\mu)$ and $E \subseteq X$ we have
    %
    \begin{align*}
        \mu(E)
            &= \mu(E \intersect A) + \mu(E \intersect A^c) \\
            &= \mu(E \intersect A) + \mu(E \intersect A^c \intersect B) + \mu(E \intersect A^c \intersect B^c) \\
            &= \mu(E \intersect (A \union B) \intersect A) + \mu(E \intersect (A \union B) \intersect A^c) + \mu(E \intersect (A \union B)^c) \\
            &= \mu(E \intersect (A \union B)) + \mu(E \intersect (A \union B)^c),
    \end{align*}
    %
    so $A \union B \in \calM(\mu)$.

    (ii) If $A_1, \ldots, A_n$ are disjoint sets from $\calM(\mu)$ and $E \subseteq X$, then
    %
    \begin{align*}
        \mu \Bigl( E \intersect \bigunion_{i=1}^n A_i \Bigr)
            &= \mu \Bigl( E \intersect \bigl( \bigunion_{i=1}^n A_i \bigr) \intersect A_n \Bigr) + \mu \Bigl( E \intersect \bigl( \bigunion_{i=1}^n A_i \bigr) \intersect A_n^c \Bigr) \\
            &= \mu(E \intersect A_n) + \mu \Bigl( E \intersect \bigunion_{i=1}^{n-1} A_i \Bigr).
    \end{align*}
    %
    The desired identity thus follows by induction, and finite additivity of $\mu$ follows by letting $E = X$.

    (iii) This is obvious.

    (iv) If $A \subseteq X$ and $\mu(A) = 0$, then for all $E \subseteq X$ we have
    %
    \begin{equation*}
        \mu(E)
            \geq \mu(E \intersect A^c)
            = \mu(E \intersect A) + \mu(E \intersect A^c),
    \end{equation*}
    %
    so it follows from (iii) that $A \in \calM(\mu)$.

    (v) Let $(A_n)$ be a sequence of disjoint sets from $\calM(\mu)$, and let $A = \bigunion_{n \in \naturals} A_n$. The inequality \enquote{$\leq$} simply holds since $\mu$ is subadditive. For the other inequality, for each $N \in \naturals$ we thus have, by (ii), that
    %
    \begin{equation*}
        \mu \Bigl( E \intersect \bigunion_{n \in \naturals} A_n \Bigr)
            \geq \mu \Bigl( E \intersect \bigunion_{n=1}^N A_n \Bigr)
            = \sum_{n=1}^N \mu(E \intersect A_n)
    \end{equation*}
    %
    for all $E \subseteq X$, and the inequality \enquote{$\geq$} thus follows by letting $N \to \infty$.
...
\begin{proposition}
    Let $\calR$ be a ring in a set $X$, and let $\mu$ be a premeasure on $\calR$. Then $\mu^*|_\calR = \mu$ and $\calR \subseteq \calM(\mu^*)$.
\end{proposition}

\begin{proof}
    We always have $\mu^*|_\calR \leq \mu$, and for the opposite inequality, let $A \in \calR$ and consider a sequence $(A_n)$ in $\calR$ with $A \subseteq \bigunion_{n \in \naturals} A_n$. Letting $B_n = A \setminus (A_1 \union \cdots \union A_{n-1})$, the $B_n$ are disjoint and their union is $A$, so
    %
    \begin{equation*}
        \mu(A)
    ...
    \end{equation*}
    %
    which implies that $\mu(A) \leq \mu^*(A)$.

    If $A \in \calR$, $E \subseteq X$ and $\epsilon > 0$, then there is a sequence $(A_n)$ in $\calR$ with $E \subseteq \bigunion_{n \in \naturals} A_n$ and $\sum_{n=1}^\infty \mu(A_n) \leq \mu^*(E) + \epsilon$. Hence,
    %
    \begin{align*}
        \mu(E) + \epsilon
             \geq \sum_{n=1}^\infty \mu(A_n)
            &= \sum_{n=1}^\infty \mu(A_n \intersect A) + \sum_{n=1}^\infty \mu(A_n \intersect A^c) \\
            &\geq \mu^*(E \intersect A) + \mu^*(E \intersect A^c),
    \end{align*}
    %
    so $A \in \calM(\mu^*)$ as desired. (Notice here that $A_n \intersect A^c = A_n \setminus A \in \calR$.)
\end{proof}
